                      ...
                      <div class="box-account box-info">
                        ...
                        <div>
                          <div class="box">
                            <div class="box-title">
                              <h3>入金口座</h3>
                              <a v-if="!stripeAccount.details_submitted" href="javascript:void(0)" @click="createStripeAccount()">入金口座登録</a>
                            </div>
                            <div class="row">
                              <div class="col-sm-6">
                                <h6>Default Billing Address</h6>
...
export default {
  ...
  mounted() {
    const stripe_id = this.$auth.user.data.stripe_id
    this.$axios.get('/api/v1/stripe/accounts/' + stripe_id).then(response => {
      this.stripeAccount = response.data
    });
  },
  methods: {
    createStripeAccount() {
      this.$axios.post('/api/v1/stripe/accounts').then(response => {
